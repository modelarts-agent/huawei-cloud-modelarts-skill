...
def ma_old_inference_service_start(access, service_id: str, workspace_id: str = '0', **kwargs) -> Dict[str, Any]:
    if not service_id: return format_api_result(False, error="service_id is required")
    print(f"  启动推理服务: {service_id}")
    body = {"status": "running", "schedule": [{"type": "stop", "time_unit": "HOURS", "duration": 1}], "workspace_id": workspace_id}
...
__all__ = ['ma_old_inference_service_start']
