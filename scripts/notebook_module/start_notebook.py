...
def start_notebook(notebook_id: str) -> Dict[str, Any]:
    """
    启动已停止的 Notebook 实例
        ...
        示例:
            >>> from notebook_module import start_notebook
            >>> result = start_notebook('notebook-xxx-xxx-xxx')
            >>> if result['success']:
            ...     print("启动成功")
    """
    ...
    try:
        ...
        print(f"  启动Notebook实例: {notebook_id}")

        def _start_notebook(session):
            from modelarts import constant
            from modelarts.config.auth import auth_by_apig
            
            request_url = f"/v1/{session.project_id}/notebooks/{notebook_id}/start"
            return auth_by_apig(session, constant.HTTPS_POST, request_url)
        
        result = access.sdk().execute(_start_notebook)

        return format_api_result(True, data=result)
        
    except Exception as e:
        return format_api_result(False, error=f"启动notebook时发生异常: {e}")


__all__ = ['start_notebook']
